test_that("loggamma precision log-prior matches Gamma(shape,rate) with the log-Jacobian", {
  a <- 0.7; b <- 0.03; lp <- ngvb:::.loggamma_prec_logprior(a, b)
...
test_that("normal precision log-prior is Gaussian on theta with (mean, precision) params", {
  lp <- ngvb:::.normal_prec_logprior(0.5, 4)          # sd = 1/2 on the log-precision scale
...
test_that("prior-family dispatch maps pc.prec/loggamma/normal and rejects the rest", {
  expect_true(ngvb:::.prec_logprior_from_hyper(list(prior = "pc.prec",  param = c(1, 0.01)))$mappable)
  expect_true(ngvb:::.prec_logprior_from_hyper(list(prior = "loggamma", param = c(1, 5e-5)))$mappable)
  expect_true(ngvb:::.prec_logprior_from_hyper(list(prior = "normal",   param = c(0, 1)))$mappable)
  expect_false(ngvb:::.prec_logprior_from_hyper(list(prior = "flat",    param = numeric(0)))$mappable)
  expect_false(ngvb:::.prec_logprior_from_hyper(list(prior = "loggamma", param = c(1, 1),
                                                      fixed = TRUE))$mappable)
})
...
test_that("hyper specs are read from fit$all.hyper, including INLA defaults", {
  ...
  hy <- ngvb:::ngvb_hyper_from_fit(fit, c("i", "j"))
  pi <- ngvb:::.find_prec_entry(hy$i)$spec
  pj <- ngvb:::.find_prec_entry(hy$j)$spec
...
test_that("applying a mapped prior replaces exactly the precision piece of logprior", {
  ...
  op2  <- suppressMessages(ngvb:::ngvb_apply_user_prec_prior(op, hy, "s", verbose = FALSE))
  want <- ngvb:::.loggamma_prec_logprior(0.1, 0.01)
...
test_that("an unmappable family and a secondary-hyperparameter prior both warn and fall back", {
  ...
  expect_warning(
    ngvb:::ngvb_apply_user_prec_prior(
...
test_that("all.hyper-style secondary hyperparameters inform rather than warn", {
  ...
  expect_no_warning(
    expect_message(
      ngvb:::ngvb_apply_user_prec_prior(op.ar, hyper, "t", verbose = TRUE),
...
test_that("a NULL hyper record warns loudly instead of silently keeping the default", {
  op0 <- ngvb_operator("iid", n = 5)
  expect_warning(
    ngvb:::ngvb_apply_user_prec_prior(op0, NULL, "z", verbose = FALSE),
